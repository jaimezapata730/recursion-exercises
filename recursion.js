...
function revString(str, idx = 0, strCopy = '') {
  if (strCopy.length === str.length) return strCopy;
  strCopy += str[str.length - 1 - idx];
  return revString(str, idx + 1, strCopy);
}
...
function gatherStrings(obj) {
  let newArr = [];
  for (let k in obj) {
    if (typeof obj[k] === 'string') newArr.push(obj[k])
    if (typeof obj[k] === 'object') newArr.push(...gatherStrings(obj[k]));
